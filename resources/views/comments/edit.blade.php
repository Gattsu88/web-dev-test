        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('comment.update', $comment->id) }}" method="POST" class="space-y-8">
                        @csrf
                        <div class="space-y-8 divide-y divide-gray-200">
                            <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">                                
                                <input type="hidden" value="{{ Auth::user()->name }}" id="moderated_by" name="moderated_by">
                                ...
                                <div class="sm:col-span-4">
                                    ...
                                    <div class="mt-1 relative rounded-md">
                                        <select name="status" id="status" class="block w-full pr-10 border-gray-300 focus:outline-none sm:text-sm rounded-md" style="width: 25%;">
                                            <option value="0" @if($comment->status == 0) selected @else "" @endif>Pending</option>
                                            <option value="1" @if($comment->status == 1) selected @else "" @endif>Approved</option>
                                            <option value="2" @if($comment->status == 2) selected @else "" @endif>Rejected</option>
                                        </select>
                                    </div>
                                </div><br><br><br>
